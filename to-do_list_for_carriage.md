...
- Add support for **table titles** in the placeholders.
- Add support for **switching files** within the same working directory as the
  current file, and therefore support for project folders.
- Add support for **selecting individual section headers** from the list in the
...
- I need to add support for the **system clipboard**.
- When selecting text using the Shift key, I should be able to use the left
  arrow to back up to the end of the previous line with the selection.
- Should I add support for **find and replace?** TBD.

## From the ChatGPT Audit

After the fixes through v1.10:

- **Table Undo/Redo is still separate from prose Undo/Redo: Medium.** Saving a
...
- **Atomic Save can still lose inode-specific metadata: Medium/Low.** Carriage
...
- **CRLF files are normalized to LF after editing: Low/Medium.** Carriage
  ...
  small edit.
- **Normal Save does not fsync the containing directory: Low.** The temporary
...
- **External-change detection fingerprints bytes, not file identity: Low.** A
  different inode containing exactly the same bytes counts as unchanged. Adding
  device/inode information to the snapshot would make replacement detection more
  complete.
- **Recovery ownership relies mainly on PID liveness: Low.** A stale recovery
  ...
  more robust.
- **Whole-document operations may become expensive on very large manuscripts:
